...
kable(fulcsec, format = "html", escape = F, caption = 'Security Factors', booktabs = TRUE, align =  "l",col.names = c("Feature"	,"Available",	"Description","Group Notes","External  Reviews")) %>%
  kable_paper() %>% 
  kable_styling(full_width =FALSE, fixed_thead = TRUE, font_size = 18) %>%
...
fulcbus %>%
  group_by(Feature) %>%
  mutate(Feature = ifelse(duplicated(Feature), "", Feature))  %>%
kable(fulcbus, format = "html", escape = F, caption = 'Business Model and Pricing',  booktabs = TRUE,col.names = c("Feature"	,"Description","Price","Group Notes","External Reviews"), align =  "l") %>%
  kable_paper() %>% 
  kable_styling(full_width =TRUE, fixed_thead = TRUE, font_size = 18) %>%
  
  column_spec(1, width_min = "4cm", bold = TRUE) %>%
  column_spec(1, width_max = "5cm", bold = TRUE) %>%
