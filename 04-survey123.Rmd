...
kable(s123sec, format = "html", escape = F, caption = 'Security Factors', booktabs = TRUE, align =  "l",col.names = c("Feature"	,"Available",	"Description","Group Notes","External  Reviews")) %>%
  kable_paper() %>% 
  kable_styling(full_width =FALSE, fixed_thead = TRUE, font_size = 18) %>%
...
s123bus %>%
  group_by(Feature) %>%
  mutate(Feature = ifelse(duplicated(Feature), "", Feature))  %>%
kable(s123bus, format = "html", escape = F, caption = 'Business Model and Pricing',  booktabs = TRUE,col.names = c("Feature"	,"Description","Price","Group Notes","External Reviews"), align =  "l") %>%
  kable_paper() %>% 
